...
class Combine(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # x = nn.Conv2d(filters, 1, padding = 'same')(x)
...
class ResUnet(nn.Module):
    def __init__(self, channel=3, filters=[32,64, 128, 256, 512,1024]):
        ...
        self.PSP = PSP(filters[4],128)
        
        self.combine1 = Combine(filters[4] + filters[3],filters[4])
        self.combine2 = Combine(filters[3] + filters[2],filters[3])
        self.combine3 = Combine(filters[2] + filters[1],filters[2])
        self.combine4 = Combine(filters[1] + filters[0],filters[1])

        self.upsample_1 = Upsample(filters[4], filters[4], 2, 2)
        self.up_residual_conv1 = ResidualConv(filters[4], filters[3], 1, 1)

        self.upsample_2 = Upsample(filters[3], filters[3], 2, 2)
        self.up_residual_conv2 = ResidualConv(filters[3], filters[2], 1, 1)

        self.upsample_3 = Upsample(filters[2], filters[2], 2, 2)
        self.up_residual_conv3 = ResidualConv(filters[2], filters[1], 1, 1)

        self.upsample_4 = Upsample(filters[1], filters[1], 2, 2)
        self.up_residual_conv4 = ResidualConv(filters[1], filters[0], 1, 1)
